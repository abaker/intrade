...
object Requests {
  ...
  def updateMultiOrder(orders: Seq[BasicOrderRequest], cancelPrevious: Boolean = false, quickCancel: Boolean = false, tif: TimeInForce = TimeInForce.Good_Til_Cancel, timeToExpire: Long = 0) =
  ...
  def multiOrderRequest(orders: Seq[AdvancedOrderRequest], cancelPrevious: Boolean = false, quickCancel: Boolean = false) =
  ...
  def orderToOrderString(order: AdvancedOrderRequest) =
  ...
  def timeInForce(order: AdvancedOrderRequest) = order.timeInForce match {
  ...
  def orderType(order: AdvancedOrderRequest) = order.orderType match {
